...
This endpoint creates a multi-stage task. Multi-stage tasks are designed to handle complex full-scene labeling that spans multiple annotation types and modalities. They serve as a replacement for Scale’s legacy dependent tasks system, which required using multiple tasks to fully label a scene.
